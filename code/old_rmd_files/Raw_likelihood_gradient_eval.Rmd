...
```{r}
s_grad2 <- function(sigma, D, latent, band.no) {
  # padding to prevent errors
  dims <- dim(latent)[2]
  n <- dim(latent)[1]
  D_latent <- as.matrix(dist(latent, diag = TRUE, upper = TRUE))
  ...
  D_latent_pad <- cbind(pad, D_latent, pad) # padding to the left & right of dist matrix
  D_latent_pnorm <- pnorm(D_latent_pad / sqrt(sigma))
  D_latent_dnorm <- dnorm(D_latent_pad / sqrt(sigma))
  D_pad <- cbind(pad, D, pad)
  
  latent_pad <- rbind(pad2, latent, pad2) # padding above & below latent variables
  ...
  for (i in 1:n){
    ...
    for (j in 1:band.no){
      ...
      #stat1 <- (D_latent_pad[i, c(i + band.no - j, i + band.no + j)]) / sqrt(sigma)
      step1.a <- (D_latent_pad[i, c(i + band.no - j, i + band.no + j)] - 
                    D_pad[i, c(i + band.no - j, i + band.no + j)]) / sigma
      #step2.a <- dnorm(stat1) / (sqrt(sigma) * pnorm(stat1))
      step2.a <- D_latent_dnorm[i, c(i + band.no - j, i + band.no + j)] /
        (sqrt(sigma) * D_latent_pnorm[i, c(i + band.no - j, i + band.no + j)])
      step3[j, ] <- (step1.a + step2.a)[1] *
        ((latent[i, ] - latent_pad[i + band.no - j, ]) / 
           D_latent_pad[i, i + band.no - j])  
      step3[j + band.no, ] <- (step1.a + step2.a)[2] *
        ((latent[i, ] - latent_pad[i + band.no + j, ]) / 
           D_latent_pad[i, i + band.no + j])
    }
...
### Raw likelihood & gradient simulation 

```{r}
set.seed(12345)
items <- c(200, 500, 1000)

ll_time <- matrix(NA, nrow = max(items), ncol = length(items))
for (i in 1:length(items)){
  Z_ex <- rmvnorm(items[i], mean = rep(0, 2), sigma = diag(2))
  D_ex <- as.matrix(dist(Z_ex, diag = TRUE, upper = TRUE))
  for (j in 1:items[i]){
    stime <- proc.time()
    sll_results <- sbmds_ll(latent = Z_ex, D = D_ex, sigmasq = 0.3, band.no = j)
    endtime <- proc.time() - stime
    ll_time[j, i] <- endtime[3]
  }
}
...
grad_time <- matrix(NA, nrow = max(items), ncol = length(items))
for (i in 1:length(items)){
  Z_ex <- rmvnorm(items[i], mean = rep(0, 2), sigma = diag(2))
  D_ex <- as.matrix(dist(Z_ex, diag = TRUE, upper = TRUE))
  for (j in 1:items[i]){
    stime <- proc.time()
    sgrad_results <- sbmds_grad(latent = Z_ex, D = D_ex, sigmasq = 0.3, band.no = j)
    endtime <- proc.time() - stime
    grad_time[j, i] <- endtime[3]
  }
}
...
#   stime <- proc.time()
#   sgrad_results <- sbmds_grad(latent = Z_ex, D = D_ex, sigmasq = 0.3, band.no = i)
#   endtime <- proc.time() - stime
...
#        x = "Number of Bands", y = "Elapsed Time (sec)") + 
#   geom_point(aes(bands, grad_time), color = "red")

## combined graph 

raw_data1 <- tibble(bands = seq(1, 200), ll_time = ll_time[1:200, 1], 
                    grad_time = grad_time[1:200, 1])  %>% 
  gather(., key = "raw type", value = "elapsed time", c(ll_time, grad_time))


raw_data2 <- tibble(bands = seq(1, 500), ll_time = ll_time[1:500, 2], 
                    grad_time = grad_time[1:500, 2])  %>% 
  gather(., key = "raw type", value = "elapsed time", c(ll_time, grad_time))

raw_data3 <- tibble(bands = seq(1, 1000), ll_time = ll_time[, 3], 
                    grad_time = grad_time[, 3])  %>% 
  gather(., key = "raw type", value = "elapsed time", c(ll_time, grad_time))

p1 <- ggplot(raw_data1, aes(bands, `elapsed time`, color = `raw type`)) + 
  geom_point() +
...
p2 <- ggplot(raw_data2, aes(bands, `elapsed time`, color = `raw type`)) + 
  geom_point() +
  #geom_smooth(aes(color = `raw type`), method = "loess") + 
  labs(title = "", x = "Number of Bands", y = "",
       color = "") +
  theme_minimal() + 
  #scale_color_discrete(labels = c("gradient", "likelihood")) +
  scale_color_manual(values = c("blue", "orange"),
...
p3 <- ggplot(raw_data3, aes(bands, `elapsed time`, color = `raw type`)) + 
  geom_point() +
  #geom_smooth(aes(color = `raw type`), method = "loess") + 
  labs(title = "", x = "Numbers of Bands", y = "",
       color = "") +
  theme_minimal() + 
  #scale_color_discrete(labels = c("gradient", "likelihood")) +
  scale_color_manual(values = c("blue", "orange"),
...
```{r}
raw_data <- read_csv("raw_like_grad_eval.txt",
         col_names = c("size", "band.no", "sd", "likelihood", "gradient"))
head(raw_data)
```

```{r}
# sample size of 200
r1 <- raw_data %>% filter(size == 200) %>% 
...
```{r}
library(ggpubr)
ggarrange(r1, r2, r3, ncol = 3, nrow = 1, common.legend = TRUE,
          legend = "bottom", labels = c("\n (N = 200)", "\n (N = 500)", 
                                        "\n (N = 1000)"),
          label.x = 0.1,
          vjust = 1.0,
