...
#   logsum = 0
#   for (i in 1:(n - 1)){
#     for (j in (i + 1):n){ # D[i, i] included so taking pnorm(0) n times 
#       stat = D_latent[i, j] / sqrt(sigma)
#       logsum = logsum + log(pnorm(stat)) 
#     }
#   }
#   #logsum = logsum - n * log(pnorm(0)) # subtract extra 0 part out 
#   return(logsum)
# }

sum_terms <- function(n, D_latent, D, sigma){
  SSR_val = 0
  logsum = 0
  for (i in 1:(n - 1)){
    for (j in (i + 1):n){
...
ll_latent <- function(sigma, D, latent){ 
  ...
  m <- n * (n - 1) / 2
  term1 <- (m / 2) * log(sigma)
  term2 <- sum_terms(n, D_latent, D, sigma)
  #term2 <- (1 / (2 * sigma)) * SSR(n, D_latent, D) 
  #term3 <- sum_norm(n, D_latent, sigma)
  #loglik <- -(term1 + term2 + term3)
  loglik <- -(term1 + term2)
...
set.seed(12345)
set.seed(12345)
Z.ex <- mvrnorm(25, mu = c(0,0), Sigma = diag(c(1, 1)))
#Z_tilde <- mvrnorm(25, mu = c(0,0), Sigma = diag(c(1, 1))) + 0.1
D.ex <- as.matrix(dist(Z_ex, diag = TRUE, upper = TRUE))

t <- proc.time()
ll_latent(sigma = 0.3, D.ex, latent = Z.ex)
proc.time() - t
...
check if gradient derived matches the numerical gradient using R package

```{r}
target3 <- function(x){
  target2(sigma = 0.3, theta = x)
}

set.seed(12345)
Z_ex <- mvrnorm(25, mu = c(0,0), Sigma = diag(c(1, 1)))
#Z_tilde <- mvrnorm(25, mu = c(0,0), Sigma = diag(c(1, 1))) + 0.1
D <- as.matrix(dist(Z_ex, diag = TRUE, upper = TRUE))

matrix(numDeriv::grad(target3, x = Z_ex), nrow = 25, ncol = 2)

grad(sigma = 0.3, D_ex, Z_ex)
```
    ...
    for (j in 1:n){
      if (i != j){
        ...
        grad_1row[j, ] <- ((D_latent[i, j] - D[i, j]) / sigma +
          dstat[i, j] / (sqrt(sigma) * pstat[i, j])) * 
