...
```{r}
set.seed(12345)
Z_ex <- rmvnorm(10, mean = rep(0, 2), sigma = diag(2))
D_ex <- as.matrix(dist(Z_ex))

# adding noise to distance matrix
noise <- matrix(rtruncnorm(10^2, mean = 0, sd = .1), 10, 10) 
...
engine_plain <- MassiveMDS::createEngine(embeddingDimension = 2, locationCount = 10,
                                   truncation = TRUE, tbb = 0, simd = 0, gpu = 0,
                                   single = 0, bandwidth = 10)
...
# traceplot for pairwise distances
s_dist_mat_hmc <- array(0, dim = c(dim(hmc_results)[1], 10, 10))
for (i in 1:1:dim(hmc_results)[1]){
  s_dist_mat_hmc[i, , ] <- as.matrix(dist(hmc_results[i, , ]))
}
...
#for (r in 1:length(samp)){
  ...
  for (s in 1:10){
    ...
    mse_band_hmc[s, 3] <- mean(error[-(1:burnin)])
  }
#}
```
