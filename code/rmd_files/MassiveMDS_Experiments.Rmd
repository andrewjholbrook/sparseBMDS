...
hmc_results <- sbmds_nonadapt_sigma_hmc(maxIts = 100000, dims = 2, data = D_noise, precision = 100,
                                        bandwidth = 25, targetAccept = 0.238, stepSize = .1)
```
...
engine_plain <- MassiveMDS::createEngine(embeddingDimension = 2, locationCount = 25, 
                                   truncation = TRUE, tbb = 0, simd = 0, gpu = 0, 
                                   single = 0, bandwidth = 25)
engine_plain <- MassiveMDS::setPairwiseData(engine_test, data = D_noise)
engine_plain <- MassiveMDS::setPrecision(engine_test, precision = 100)
