...
# traceplot for pairwise distances
s_dist_mat_hmc <- array(0, dim = c(50, 10000, 10000))#c(dim(hmc_results)[1], 10000, 10000))
j = 0
for (i in 1:100000){
  if (i %% 1000 == 0){
    j = j + 1
...
for (i in 1:5){
  for (j in 1:5){
    if (i != j){
      plot(s_dist_mat_hmc[1:100, i, j], type = "l")
      abline(h = D_ex[i, j], col = "red")
    }
  }
}
...
#for (r in 1:length(samp)){
  ...
  for (s in 1:2){
    ...
    sim <- sbmds_nonadapt_sigma_hmc(maxIts = 100, dims = 2, data = D.noise, 
    ...
    mse_band_hmc_10000[s] <- mean(error[-(1:burnin)])
  }
#}
```
