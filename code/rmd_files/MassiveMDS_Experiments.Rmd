...
noise <- matrix(rtruncnorm(100^2, mean = 0, sd = .1), 1000, 1000) 
...
hmc_results <- sbmds_nonadapt_sigma_hmc(maxIts = 100000, dims = 2, data = D_noise, precision = 100,
                                        bandwidth = 10, targetAccept = 0.238, stepSize = .1)
```
...
# traceplot for pairwise distances
s_dist_mat_hmc <- array(0, dim = c(1001, 1000, 1000))
for (i in 1:100000){
  s_dist_mat_hmc[i - 98999, , ] <- as.matrix(dist(hmc_results[i, , ]))
}
...
for (i in 1:5){
  for (j in 1:5){
    if (i != j){
      plot(s_dist_mat_hmc[-(1:20000), i, j], type = "l")
      abline(h = D_ex[i, j], col = "red")
    }
  }
}
